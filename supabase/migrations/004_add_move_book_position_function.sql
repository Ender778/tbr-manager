-- Function to atomically move a book position while handling conflicts
...
BEGIN
  -- Start with a temporary position to avoid conflicts
  temp_position := -1;
  ...
  ELSE
    ...
    IF p_new_position > p_current_position THEN
      -- Moving down: shift positions between current and new position up
      UPDATE book_positions 
      SET position = position - 1, updated_at = NOW()
      WHERE shelf_id = p_to_shelf_id 
        AND user_id = p_user_id
    ...
    ELSIF p_new_position < p_current_position THEN
      -- Moving up: shift positions between new and current position down
      UPDATE book_positions 
      SET position = position + 1, updated_at = NOW()
      WHERE shelf_id = p_to_shelf_id 
        AND user_id = p_user_id
